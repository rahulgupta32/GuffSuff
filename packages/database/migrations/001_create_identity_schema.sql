-- Migration 001: Create Identity Schema for GuffSuff Phase 4
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_phone_identities_verified_blind 
  ON phone_identities(phone_blind_index) WHERE verified_at IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_phone_identities_user_id ON phone_identities(user_id);
...
CREATE INDEX IF NOT EXISTS idx_otp_challenges_phone ON otp_challenges(phone_blind_index);
...
CREATE TABLE IF NOT EXISTS registration_lock_credentials (
  user_id UUID PRIMARY KEY REFERENCES users(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS registration_lock_attempts (
  id UUID PRIMARY KEY,
  user_id UUID NOT NULL REFERENCES users(id) ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS idx_reg_lock_attempts_user ON registration_lock_attempts(user_id);
