...
void main() {
  ...
  group('Android Native Provider-Neutral Boundary Integration Tests', () {
    ...
    testWidgets('1. Native library loads successfully', (
      WidgetTester tester,
    ) async {
      expect(provider.isAvailable, isTrue);
    });

    testWidgets('2. Native API version matches expected boundary API version', (
      WidgetTester tester,
    ) async {
      expect(provider.apiVersion, equals(1));
    });

    testWidgets('3. Capability query returns expected test capabilities', (
      WidgetTester tester,
    ) async {
    ...
    testWidgets('5. Session handle is created', (WidgetTester tester) async {
      final handle = await provider.establishOutboundSession(
        'test_prekey_bundle',
      );
      expect(handle.handleId, greaterThan(0));
    });
    ...
    testWidgets('6. Group handle is created', (WidgetTester tester) async {
      final handle = await provider.createGroupState('group_123', [
        'device_a',
        'device_b',
      ]);
      expect(handle.handleId, greaterThan(0));
    ...
    testWidgets(
      '7 & 8 & 9. Byte input crosses into native code, returns output labeled test-only',
      (WidgetTester tester) async {
        ...
        expect(
          ...
          contains('TEST PROVIDER — NO CONFIDENTIALITY OR AUTHENTICITY'),
        );
      },
    );

    testWidgets('10 & 13. Opaque state exports and imports successfully', (
      WidgetTester tester,
    ) async {
      final handle = await provider.establishOutboundSession(
        'test_state_bundle',
      ...
      final sessionEntry = parsed.firstWhere(
        (h) => h.handleId == handle.handleId,
      );
      ...
      expect(
        ...
        throwsA(
          predicate((e) => e.toString().contains('STALE_OR_DOUBLE_FREE')),
        ),
      );
    });

    testWidgets(
      '14 & 15. Handle is disposed and reuse fails with STALE_OR_DOUBLE_FREE',
      (WidgetTester tester) async {
    ...
    testWidgets('16. Unknown handle fails with INVALID_HANDLE', (
      WidgetTester tester,
    ) async {
    ...
    testWidgets('17. Double disposal fails with STALE_OR_DOUBLE_FREE', (
      WidgetTester tester,
    ) async {
    ...
    testWidgets('18. Empty buffer behavior is defined', (
      WidgetTester tester,
    ) async {
      final handle = await provider.establishOutboundSession('bundle');
    ...
    testWidgets('19. Oversized buffer fails with ArgumentError', (
      WidgetTester tester,
    ) async {
      final handle = await provider.establishOutboundSession('bundle');
    ...
    testWidgets(
      '20 & 21. Controlled native panic is contained and mapped to error code -4',
      (WidgetTester tester) async {
    ...
    testWidgets('22. Concurrent calls do not corrupt registry state', (
      WidgetTester tester,
    ) async {
      final futures = List.generate(20, (_) async {
        final handle = await provider.establishOutboundSession('b');
        final res = await provider.encryptPayload(
          handle,
          Uint8List.fromList('Concurrent'.codeUnits),
        );
        await provider.disposeHandle(handle);
        return res;
      });
    ...
    testWidgets(
      '23 & 24. Provider capability safety assertions enforce production rules',
      (WidgetTester tester) async {
        final caps = provider.queryCapabilities();
        expect(caps.isTestProvider, isTrue);
        expect(
    ...
    testWidgets(
      '25 & 26. Unavailable crypto provider returns fail-closed state',
      (WidgetTester tester) async {
    ...
    testWidgets('27. Canary payload is processed without being logged', (
      WidgetTester tester,
    ) async {
    ...
    testWidgets(
      '28. Identity handle cannot be used as session handle (returns ERR_HANDLE_TYPE_MISMATCH)',
      (WidgetTester tester) async {
        ...
        expect(
          ...
          throwsA(
            predicate((e) => e.toString().contains('ERR_HANDLE_TYPE_MISMATCH')),
          ),
        );
    ...
    testWidgets(
      '29. Group handle cannot be used as session handle (returns ERR_HANDLE_TYPE_MISMATCH)',
      (WidgetTester tester) async {
