...
void main() {
  group('Provider-Neutral Mobile Boundary Contract & Safety Tests', () {
    test('UnavailableCryptoProvider fails closed on all operations', () async {
      ...
      expect(() => provider.queryCapabilities(), throwsA(isA<ProviderUnavailableException>()));
      expect(() => provider.initializeDeviceIdentity(), throwsA(isA<ProviderUnavailableException>()));
      expect(() => provider.establishOutboundSession('bundle_001'), throwsA(isA<ProviderUnavailableException>()));
      expect(() => provider.createGroupState('grp_001', ['dev_1']), throwsA(isA<ProviderUnavailableException>()));
      expect(() => provider.encryptPayload(const OpaqueSessionHandle(1), [1, 2, 3]), throwsA(isA<ProviderUnavailableException>()));
      expect(() => provider.decryptPayload(const OpaqueSessionHandle(1), [1, 2, 3]), throwsA(isA<ProviderUnavailableException>()));
      expect(() => provider.disposeHandle(const OpaqueSessionHandle(1)), throwsA(isA<ProviderUnavailableException>()));
    });

    test('assertProductionProviderSafety hard-rejects test provider in production mode', () {
      ...
      expect(() => assertProductionProviderSafety(caps, false), returnsNormally);
    ...
    test('TestBoundaryCryptoProvider manages opaque handle lifecycles', () async {
